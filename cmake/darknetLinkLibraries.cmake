...
target_link_libraries(darknet PUBLIC dark)
...
if (OpenCV_FOUND)
    if (OpenCV_VERSION VERSION_GREATER "3.0" AND BUILD_USELIB_TRACK AND NOT MANUALLY_EXPORT_TRACK_OPTFLOW)
        target_compile_definitions(dark PUBLIC TRACK_OPTFLOW=1)
        target_compile_definitions(dark_opencv PUBLIC TRACK_OPTFLOW=1)
    endif ()

    # target_compile_definitions(dark PUBLIC -DDARKNET_USE_OPENCV)

    target_compile_definitions(darknet_opencv PRIVATE -DUSE_CMAKE_LIBS)
    target_compile_definitions(dark_opencv PRIVATE -DUSE_CMAKE_LIBS)

    target_link_libraries(darknet_opencv PUBLIC dark_opencv)
    target_link_libraries(dark_opencv PUBLIC ${OpenCV_LINKED_COMPONENTS})

    target_include_directories(dark_opencv PUBLIC ${OpenCV_INCLUDE_DIRS})
    target_link_libraries(dark_opencv PUBLIC dark)
    target_link_libraries(dark_opencv PUBLIC Threads::Threads)

    if (ENABLE_CUDA)
        target_include_directories(darknet_opencv PRIVATE ${CMAKE_CUDA_TOOLKIT_INCLUDE_DIRECTORIES})
        target_include_directories(dark_opencv PUBLIC ${CMAKE_CUDA_TOOLKIT_INCLUDE_DIRECTORIES})
        target_link_libraries(darknet_opencv PRIVATE curand cublas cudart cuda)
        target_link_libraries(dark_opencv PRIVATE curand cublas cudart cuda)
        set_target_properties(dark_opencv PROPERTIES CUDA_RESOLVE_DEVICE_SYMBOLS ON)
        target_compile_definitions(darknet_opencv PRIVATE -DGPU)
        target_compile_definitions(dark_opencv PUBLIC -DGPU)
    endif ()

    if (OPENMP_FOUND)
        target_link_libraries(darknet_opencv PUBLIC OpenMP::OpenMP_CXX)
        target_link_libraries(darknet_opencv PUBLIC OpenMP::OpenMP_C)
        target_link_libraries(dark_opencv PUBLIC OpenMP::OpenMP_CXX)
        target_link_libraries(dark_opencv PUBLIC OpenMP::OpenMP_C)
    endif (OPENMP_FOUND)
    if (CMAKE_COMPILER_IS_GNUCC)
        target_link_libraries(darknet_opencv PUBLIC m)
        target_link_libraries(dark_opencv PUBLIC m)
    endif ()
    if (MSVC)
        target_link_libraries(darknet_opencv PUBLIC PThreads_windows::PThreads_windows)
        target_link_libraries(darknet_opencv PUBLIC wsock32)
        target_compile_definitions(darknet_opencv PRIVATE -D_CRT_RAND_S -DNOMINMAX -D_USE_MATH_DEFINES)
        target_compile_definitions(darknet_opencv PUBLIC -D_CRT_SECURE_NO_WARNINGS)
        target_link_libraries(dark_opencv PUBLIC PThreads_windows::PThreads_windows)
        target_link_libraries(dark_opencv PUBLIC wsock32)
        target_compile_definitions(dark_opencv PRIVATE -D_CRT_RAND_S -DNOMINMAX -D_USE_MATH_DEFINES)
        target_compile_definitions(dark_opencv PUBLIC -D_CRT_SECURE_NO_WARNINGS)
    endif ()
    if (MSVC OR MINGW)
        target_link_libraries(darknet_opencv PUBLIC ws2_32)
        target_link_libraries(dark_opencv PUBLIC ws2_32)
    endif ()
endif ()

if (ENABLE_REALSENSE)
    target_link_libraries(darknet PUBLIC ${realsense2_LINKED_COMPONENTS})

    target_compile_definitions(dark_realsense PRIVATE -DUSE_CMAKE_LIBS)
    message("Realsense libraries ${realsense2_LINKED_COMPONENTS}")
    target_link_libraries(dark_realsense PUBLIC ${realsense2_LINKED_COMPONENTS})
    target_link_libraries(dark_realsense PUBLIC dark dark_opencv)
    if (OPENCV_FOUND)
        target_compile_definitions(dark_realsense PUBLIC -DDARKNET_USE_OPENCV)
        target_link_libraries(dark_realsense PUBLIC ${OpenCV_LINKED_COMPONENTS})
        target_link_libraries(dark_realsense PUBLIC Threads::Threads)
    endif ()
    if (OPENMP_FOUND)
        target_link_libraries(dark_realsense PUBLIC OpenMP::OpenMP_CXX)
        target_link_libraries(dark_realsense PUBLIC OpenMP::OpenMP_C)
    endif (OPENMP_FOUND)
    if (CMAKE_COMPILER_IS_GNUCC)
        target_link_libraries(dark_realsense PUBLIC m)
    endif ()
    if (MSVC)
        target_link_libraries(dark_realsense PUBLIC PThreads_windows::PThreads_windows)
        target_link_libraries(dark_realsense PUBLIC wsock32)
        target_compile_definitions(dark_realsense PRIVATE -D_CRT_RAND_S -DNOMINMAX -D_USE_MATH_DEFINES)
        target_compile_definitions(dark_realsense PUBLIC -D_CRT_SECURE_NO_WARNINGS)
    endif ()
    if (MSVC OR MINGW)
        target_link_libraries(dark_realsense PUBLIC ws2_32)
    endif ()
endif ()
...
if (ENABLE_CUDA)
    ...
    target_link_libraries(darknet PRIVATE curand cublas cudart cuda)
    target_link_libraries(dark PRIVATE curand cublas cudart cuda)
