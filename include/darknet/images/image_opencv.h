...
void show_image_cv_layers(image p, char *name);
void show_image_cv_collapsed(image p, char *name);
void show_image_cv_normalized(image im, const char *name);
void show_images_cv(image *ims, int n, char *window);
...
image process_image_and_depth(int w, int h, int c, const mat_cv *srcImg, mat_cv **inImg, const void *srcDepth,
                              void **inDepth, int letterbox);
image process_image_and_depth_explicit(int w, int h, int c, const mat_cv *srcImg, mat_cv **inImg, const void *srcDepth,
...
void draw_train_loss(char *windows_name, mat_cv *img, int img_size, float avg_loss, float max_img_loss,
                     int current_batch, int max_batches, float precision, int draw_precision, char *accuracy_name,
                     float contr_acc, int dont_show, int mjpeg_port, double time_remaining);
...
// Show Anchors
void show_anchors(int number_of_boxes, int num_of_clusters, float *rel_width_height_array, model anchors_data,
                  int width, int height);

void show_opencv_info();

void test_resize_cv(char *filename);
