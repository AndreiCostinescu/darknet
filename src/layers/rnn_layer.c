...
layer make_rnn_layer(int batch, int inputs, int hidden, int outputs, int steps, ACTIVATION activation,
    ...
    *(l.input_layer) = make_connected_layer(batch, steps, inputs, hidden, activation, batch_normalize, verbose);
    ...
    *(l.self_layer) = make_connected_layer(
            batch, steps, hidden, hidden, (log == 2) ? LOGGY : (log == 1 ? LOGISTIC : activation), batch_normalize,
            verbose);
    ...
    *(l.output_layer) = make_connected_layer(batch, steps, hidden, outputs, activation, batch_normalize, verbose);
