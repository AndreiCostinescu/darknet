...
// Video Writer
// ====================================================================
extern "C" write_cv *create_video_writer(char *out_filename, char c1, char c2, char c3, char c4, int fps, int width,
...
extern "C" image process_image_and_depth_explicit(
    ...
    if (letterbox) {
        ...
        if (c > 1) cv::cvtColor(*src, *src, cv::COLOR_RGB2BGR);
        image tmp = mat_to_image_cv((mat_cv *) src);
        ...
        cv::resize(*src, new_img, new_img.size(), 0, 0, cv::INTER_LINEAR);
        if (c > 1) cv::cvtColor(new_img, new_img, cv::COLOR_RGB2BGR);
        im = mat_to_image_cv((mat_cv *) &new_img);
    }
    return im;
}
...
extern "C" void draw_detections_cv_with_depth(
    ...
    int use_depth = (_getDepth != nullptr);
    auto *getDepth = (std::function<double(int, int, int, int)> *) _getDepth;
    ...
    try {
        ...
        // printf("draw_detections: ShowImg = %d\n", show_img);
        int i, j;
        if (!show_img) return;
        // printf("draw_detections: Show Img size (%d x %d)\n", show_img->rows, show_img->cols);
        ...
        for (i = 0; i < num; ++i) {
            ...
            if (class_id >= 0) {
                int width = (int) std::max(1.0f, (float) show_img->rows * .002f);
                ...
                if (use_depth) {
                    depth = (*getDepth)(box_x_center, box_y_center, show_img->rows, show_img->cols);
                ...
                pt_text.y = top - 4;// 12;
                pt_text_bg1.x = left;
                pt_text_bg1.y = (int) ((float) top - (3 + 18 * font_size));
                pt_text_bg2.x = right;
...
// ====================================================================
extern "C" mat_cv *draw_train_chart(char *windows_name, float max_img_loss, int max_batches, int number_of_lines,
                                    int img_size, int dont_show, char *chart_path) {
    int img_offset = 60;
...
extern "C" void cv_draw_object(image sized, float *truth_cpu, int max_boxes, int num_truth, int *it_num_set,
                               float *lr_set, int *boxonly, int classes, char **names) {
...
// Show Anchors
// ====================================================================
extern "C" void show_anchors(int number_of_boxes, int num_of_clusters, float *rel_width_height_array,
                             model anchors_data, int width, int height) {
